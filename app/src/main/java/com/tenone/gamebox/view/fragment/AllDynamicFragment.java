...
public class AllDynamicFragment extends BaseLazyFragment implements HttpResultListener, SwipeRefreshLayout.OnRefreshListener, RefreshLayout.OnLoadListener, DynamicOperationListener, OnHeaderClickListener, AdapterView.OnItemClickListener, PlatformActionListener, OnLoginStateChangeListener, OnDataRefreshListener {
    ...
    public void onRefresh() {
        hasData = true;
        page = 1;
        HttpManager.getDynamics( HttpType.REFRESH, getActivity(), this, page, 2 );
    ...
    public void onLoad() {
        if (hasData) {
            page++;
            HttpManager.getDynamics( HttpType.LOADING, getActivity(), this, page, 2 );
        } else {
            refreshLayout.setOnLoadListener( null );
    ...
    public void onShareClick(final DynamicModel model) {
        ...
        sharePopupWindow.setPlatformActionListener( AllDynamicFragment.this );
    ...
    public void onDataRefresh(int which) {
        if (which == OnDataRefreshListener.ALL) {
            refreshLayout.setRefreshing( true );
            hasData = true;
            page = 1;
            HttpManager.getDynamics( HttpType.REFRESH, getActivity(), this, page, 2 );
