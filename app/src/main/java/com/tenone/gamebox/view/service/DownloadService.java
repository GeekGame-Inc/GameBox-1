...
import com.tenone.gamebox.view.receiver.NetworkChangeRexeiver.OnNetworkChangeListener;
...
public class DownloadService extends Service implements
		OnNetworkChangeListener, DeleteDialogConfrimListener,
		HttpResultListener {
	...
	public int onStartCommand(Intent intent, int flags, int startId) {
		...
		if (intent != null && intent.hasExtra( "gameModel" )) {
			...
			if (data.getStatus() == GameStatus.LOADING) {
				if (BasePresenter.is185()) {
					startDispose( data );
				} else {
					BasePresenter.requestChannelDownload(
							BasePresenter.CHANNELDOWNLOAD, data,
	...
	private void startDispose(GameModel gameModel) {
		if (!TextUtils.isEmpty( gameModel.getUrl() )) {
			DownThread thread = manager.getThreads().get( gameModel.getUrl() );
			if (thread != null) {
				thread.setGameModel( gameModel );
				thread.run();
			} else {
				thread = new DownThread( gameModel );
				thread.setName( gameModel.getUrl() );
				manager.addThreads( thread );
				thread.start();
			}
		} else {
	...
	public class DownThread extends Thread implements
			OnFileDownloadStatusListener, OnDeleteDownloadFileListener,
			HttpResultListener {
		GameModel gameModel;

		public DownThread(GameModel model) {
	...
	public void networkChange(boolean isAvailable, boolean isWifi) {
        ...
        if (isAvailable && SpUtil.getWifi() && !isWifi) {
            if (manager != null) {
                for (Entry<String, DownThread> entry : manager.getThreads()
                        .entrySet()) {
	...
	public void onSuccess(int what, ResultItem resultItem) {
		switch (what) {
			case BasePresenter.CHANNELDOWNLOAD:
				if ("0".equals( resultItem.getString( "status" ) )) {
					ResultItem item = resultItem.getItem( "data" );
					if (item != null) {
						data.setUrl( item.getString( "download_url" ) );
						startDispose( data );
					} else {
						Toast.makeText( getApplicationContext(),
								getApplicationContext().getResources().getString( R.string.no_download_path ),
								Toast.LENGTH_SHORT ).show();
					}
				} else {
					sendBroadcast( Configuration.deleteFilter, data );
					Toast.makeText( getApplicationContext(),
	...
	private void sendNotification(String gameName, String gameId, String apkName) {
		if (SpUtil.getNotification()) {
			...
			Notification notification = builder
					...
					.setSmallIcon( R.drawable.d_icon_hezi )
