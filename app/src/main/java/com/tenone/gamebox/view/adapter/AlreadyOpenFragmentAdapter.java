...
public class AlreadyOpenFragmentAdapter extends BaseAdapter {
	...
	public AlreadyOpenFragmentAdapter(List<OpenServerMode> list, Context cxt) {
		this.items = list;
		this.mContext = cxt;
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		AlreadyOpenFragmentHolder holder = null;
		if (convertView == null) {
			convertView = mInflater.inflate( R.layout.item_today_open_list,
					parent, false );
			holder = new AlreadyOpenFragmentHolder( convertView );
			convertView.setTag( holder );
		} else {
			holder = (AlreadyOpenFragmentHolder) convertView.getTag();
		}
		...
		imgparams.width = DisplayMetricsUtils.getScreenHeight( mContext ) /
				(DisplayMetricsUtils.getScreenHeight( mContext ) / DisplayMetricsUtils.getScreenWidth( mContext ) == 16 / 9 ?
						11 : 12);
	...
	private class AlreadyOpenFragmentHolder {
		ImageView imageView;
		...
		public AlreadyOpenFragmentHolder(View v) {
			initView( v );
		}
