...
public class UnderlinePageIndicator extends View implements PageIndicator {
    ...
    private float defultWidth = 0;
    private float defultOffset = 0;
    private float defultHeigth = 0;
    ...
    public UnderlinePageIndicator(Context context, AttributeSet attrs,
        ...
        defultWidth = a.getDimension(
                R.styleable.UnderlinePageIndicator_lineWidth, 0 );
        defultHeigth = a.getDimension(
                R.styleable.UnderlinePageIndicator_lineHeight, 0 );
        if (background != null) {
    ...
    public float getDefultWidth() {
        return defultWidth;
    }

    public void setDefultWidth(float defultWidth) {
        this.defultWidth = defultWidth;
        invalidate();
    }

    public float getDefultOffset() {
        return defultOffset;
    }

    public void setDefultOffset(float defultOffset) {
        this.defultOffset = defultOffset;
        invalidate();
    }
    ...
    protected void onDraw(Canvas canvas) {
       ...
       // left is 0.0 right is 153.0 mCurrentPage is 0 mPositionOffset is 0.0  defultWidth is 153.0
        //left is 201.0 right is 354.0 mCurrentPage is 1 mPositionOffset is 0.0  defultWidth is 153.0
        final int paddingLeft = getPaddingLeft();
        final float width = ((indicatorWidth == 0 ? getWidth() : indicatorWidth) - paddingLeft - getPaddingRight())
                / (1f * count);
        final float pageWidth = defultWidth == 0 ? width : defultWidth;

        final float left = paddingLeft + width
                * (mCurrentPage + mPositionOffset) + defultOffset;

        final float right = left + pageWidth;
        Log.i( "underline", " left is " + left + " right is " + right + " mCurrentPage is " + mCurrentPage + " mPositionOffset is " + mPositionOffset + " defultWidth is " + defultWidth );

        final float top = getPaddingTop();
        final float bottom = defultHeigth == 0 ? getHeight()
                - getPaddingBottom() : defultHeigth;
