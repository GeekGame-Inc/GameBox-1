...
public class TabPageIndicator extends HorizontalScrollView implements
    ...
    public void setTabViewWidth(float tabViewWidth) {
        this.tabViewWidth = tabViewWidth;
    }
    ...
    private void addTab(int index, CharSequence text, int iconResId) {
        ...
        for (int i = 0; i < array.length(); i++) {
            ...
            switch (att) {
                case R.styleable.ViewPagerIndicator_vpiTitlePageIndicatorStyle:
        ...
        int defultSize = (int) TypedValue.applyDimension(
                TypedValue.COMPLEX_UNIT_SP, 15, getResources()
                        .getDisplayMetrics() );
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            tabView.setTextSize( 14 );
        } else {
            tabView.setTextSize( defultSize / 2 );
        }
        if (!TextUtils.isEmpty( text )) {
            textWidth = tabView.getPaint().measureText( text.toString() );
        }
        if (iconResId != 0) {
            tabView.setCompoundDrawablesWithIntrinsicBounds( iconResId, 0, 0, 0 );
        }
        if (tabView != null) {
            tabViewWidth = textWidth + tabView.getPaddingLeft() + tabView.getPaddingRight();
        }
