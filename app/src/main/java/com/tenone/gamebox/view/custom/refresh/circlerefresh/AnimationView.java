package com.tenone.gamebox.view.custom.refresh.circlerefresh;
...
public class AnimationView extends View {
	...
	private void initView(Context context, AttributeSet attrs, int defStyleAttr) {
		...
		PULL_HEIGHT = (int) TypedValue.applyDimension(
				TypedValue.COMPLEX_UNIT_DIP, 100, context.getResources()
						.getDisplayMetrics());
		PULL_DELTA = (int) TypedValue.applyDimension(
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		if (height > PULL_DELTA + PULL_HEIGHT) {
			heightMeasureSpec = MeasureSpec.makeMeasureSpec(PULL_DELTA
					+ PULL_HEIGHT, MeasureSpec.getMode(heightMeasureSpec));
		}
	...
	private void drawDrag(Canvas canvas) {
		...
		mPath.quadTo(mWidthOffset * mWidth, PULL_HEIGHT
				+ (mHeight - PULL_HEIGHT) * 2, mWidth, PULL_HEIGHT);
	...
	private void drawSpring(Canvas canvas, int springDelta) {
		...
		if (curH > PULL_HEIGHT - PULL_DELTA / 2) {
			int leftX = (int) (mWidth / 2 - 2 * mRadius + getSprRatio()
					* mRadius);
			mPath.reset();
			mPath.moveTo(leftX, curH);
			mPath.quadTo(mWidth / 2, curH - mRadius * getSprRatio() * 2, mWidth
					- leftX, curH);
			canvas.drawPath(mPath, mBallPaint);
		} else {
			canvas.drawArc(new RectF(mWidth / 2 - mRadius, curH - mRadius,
					mWidth / 2 + mRadius, curH + mRadius), 180, 180, true,
					mBallPaint);
		}
	...
	private void drawPopBall(Canvas canvas) {
		...
		canvas.drawArc(new RectF(mWidth / 2 - mRadius, cirCenY - mRadius,
				mWidth / 2 + mRadius, cirCenY + mRadius), 180, 360, true,
				mBallPaint);
	...
	private void drawOutCir(Canvas canvas) {
		...
		mPath.moveTo(0, 0);
		mPath.lineTo(0, PULL_HEIGHT);
		mPath.quadTo(mWidth / 2,
				PULL_HEIGHT - (1 - getOutRatio()) * PULL_DELTA, mWidth,
				PULL_HEIGHT);
		mPath.lineTo(mWidth, 0);
		canvas.drawPath(mPath, mBackPaint);
	...
	private void drawRefreshing(Canvas canvas) {
		...
		canvas.drawArc(new RectF(mWidth / 2 - outerR, innerY - outerR, mWidth
				/ 2 + outerR, innerY + outerR), mRefreshStart, swipe, false,
				mOutPaint);
	...
	private void drawDone(Canvas canvas) {
		...
		if (getDoneRatio() < 0.3) {
			...
			int afterColor = Color.argb(
					(int) (0xff * (1 - getDoneRatio() / 0.3f)),
					Color.red(beforeColor), Color.green(beforeColor),
					Color.blue(beforeColor));
			mOutPaint.setColor(afterColor);
			canvas.drawArc(new RectF(mWidth / 2 - outerR, innerY - outerR,
					mWidth / 2 + outerR, innerY + outerR), 0, 360, false,
					mOutPaint);
		}
		...
		if (getDoneRatio() >= 0.3 && getDoneRatio() < 0.7) {
			...
			int curCentY = (int) (startCentY + (PULL_DELTA / 2 + mRadius * 2)
					* fraction);
	...
	private float getRelRatio() {
		...
		float ratio = (System.currentTimeMillis() - mStart)
				/ (float) REL_DRAG_DUR;
	...
	private float getSprRatio() {
		...
		float ratio = (System.currentTimeMillis() - mSprStart)
				/ (float) SPRING_DUR;
	...
	private float getPopRatio() {
		...
		float ratio = (System.currentTimeMillis() - mPopStart)
				/ (float) POP_BALL_DUR;
		return Math.min(ratio, 1);
	}
	...
	private float getOutRatio() {
		...
		float ratio = (System.currentTimeMillis() - mOutStart)
				/ (float) OUTER_DUR;
		return Math.min(ratio, 1);
	}
	...
	private float getDoneRatio() {
		...
		float ratio = (System.currentTimeMillis() - mDoneStart)
				/ (float) DONE_DUR;
		return Math.min(ratio, 1);
	}
