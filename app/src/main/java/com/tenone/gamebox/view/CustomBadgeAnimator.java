package com.tenone.gamebox.view;
...
public class CustomBadgeAnimator extends ValueAnimator {
    private BitmapFragment[][] mFragments;
    private WeakReference<CustomQBadgeView> mWeakBadge;

    public CustomBadgeAnimator(Bitmap badgeBitmap, PointF center, CustomQBadgeView badge) {
        mWeakBadge = new WeakReference<>(badge);
        setFloatValues(0f, 1f);
        setDuration(500);
        mFragments = getFragments(badgeBitmap, center);
        addUpdateListener( animation -> {
						CustomQBadgeView badgeView = mWeakBadge.get();
